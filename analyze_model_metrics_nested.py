#!/usr/bin/env python3
"""
Script to analyze model metrics from the Comorbidities-Detection models directory with nested structure.
Extracts training progress information and generates a comprehensive CSV summary with flags based on subdirectories.
This script handles the older directory structure with biomarker config types and nested subdirectories.
"""
...
def scan_models_directory(models_dir: str) -> List[Dict[str, Any]]:
    """
    Scan the models directory and extract metrics from all model runs with nested structure.
    This function handles the older directory structure with biomarker config types and flags.
...
def main():
    ...
    if args.output_filename:
        ...
        output_filename = f"model_metrics_nested_{timestamp}.csv"
    
    output_path = os.path.join(output_dir, output_filename)
    
    print(f"Starting nested model metrics analysis...")
