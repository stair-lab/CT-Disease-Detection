    ...
    with torch.no_grad():
        ...
        for i, val in tqdm(enumerate(test_dataloader)):
            ...
            for j in range(1, num_classes):
                ...
                pass
            del val, prediction, labels, img

        print(y_pred.shape)

        for j in range(num_classes):
            class_score[j] = roc_auc_score(y_true[:, j], y_pred[:, j])

    return result, 100 * tot_score/(i + 1), class_score, stats, correct/len(test_dataloader.dataset)
...
if not args.only_pred:
    print('AUROC Score:')
    for i in range(num_classes):
        print('{}: {:.4f}'.format(conditions[i], class_score[i]))
if args.calc_stats:
