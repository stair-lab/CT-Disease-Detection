...
BASE_CMD="$PYTHON_PATH run_experiments.py \
    ...
    --output_base_dir /lfs/turing1/0/mahmedc/Comorbidities-Detection/models/mlhc_targets \
    --resume \
    --no_confirm \
    --epochs 100"
...
for i in "${!LEARNING_RATES[@]}"; do
    ...
    tmux send-keys -t "$SESSION_NAME" "echo \"🚀 Starting ResNet-34 MLHC Targets - LR: $LR, GPU: $GPU\"" Enter
...
echo "  tmux list-sessions | grep efficientnet_b0_reg"
echo ""
echo "🔍 Attach to specific experiment:"
for i in "${!LEARNING_RATES[@]}"; do
    LR="${LEARNING_RATES[$i]}"
    GPU="${AVAILABLE_GPUS[$i]}"
    SESSION_NAME="efficientnet_b0_reg_lr_${LR}_gpu_${GPU}"
