...
echo "🚀 LAUNCHING PARALLEL ResNet-34 MLHC Targets Linear Probe Only (Modified) EXPERIMENTS"
echo "=============================================================="
echo "🎯 Model: ResNet-34 (MLHC Targets Linear Probe Only (Modified))"
...
echo "🏃 Starting parallel ResNet-34 MLHC Targets Linear Probe Only (Modified) experiments..."
...
BASE_CMD="$PYTHON_PATH run_experiments.py \
    ...
    --output_base_dir /lfs/turing1/0/mahmedc/Comorbidities-Detection/models/mlhc_targets/linear_probe_modified \
    --resume \
    --no_confirm \
    --epochs 100"
...
for i in "${!LEARNING_RATES[@]}"; do
    ...
    SESSION_NAME="resnet_34_mlhc_targets_linear_probe_modified_lr_${LR}_gpu_${GPU}"
    ...
    tmux send-keys -t "$SESSION_NAME" "echo \"🚀 Starting ResNet-34 MLHC Targets Linear Probe Only (Modified) - LR: $LR, GPU: $GPU\"" Enter
...
echo "  tmux list-sessions | grep resnet_34_mlhc_targets_linear_probe_modified"
echo ""
...
echo ""
echo "📁 Results will be saved in: /lfs/turing1/0/mahmedc/Comorbidities-Detection/models/mlhc_targets/linear_probe_modified"
