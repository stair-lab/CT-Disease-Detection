...
echo "=============================================================="
echo "🎯 Model: ResNet-34 (MLHC Targets Without Age)"
...
BASE_CMD="$PYTHON_PATH run_experiments.py \
    ...
    --biomarker_config config/biomarker_config_mlhc_wo_age.yaml \
    --data_dir /lfs/turing1/0/mahmedc/Comorbidities-Detection/datasets/full_data \
    --model_name \"$MODEL_NAME\" \
    --output_base_dir /lfs/turing1/0/mahmedc/Comorbidities-Detection/models/mlhc_targets_wo_age \
    --resume \
    --no_confirm \
    --epochs 100"
...
for i in "${!LEARNING_RATES[@]}"; do
    ...
    SESSION_NAME="resnet_34_mlhc_targets_wo_age_lr_${LR}_gpu_${GPU}"
    ...
    tmux send-keys -t "$SESSION_NAME" "echo \"🚀 Starting ResNet-34 MLHC Targets Without Age - LR: $LR, GPU: $GPU\"" Enter
...
echo "  tmux list-sessions | grep resnet_34_mlhc_targets_wo_age"
echo ""
...
echo ""
echo "📁 Results will be saved in: /lfs/turing1/0/mahmedc/Comorbidities-Detection/models/mlhc_targets_wo_age"
