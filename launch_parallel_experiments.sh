...
echo "🚀 LAUNCHING PARALLEL ResNet-34 Calcium Scoring EXPERIMENTS"
echo "=============================================================="
echo "🎯 Model: ResNet-34 (Calcium Scoring)"
...
echo "🏃 Starting parallel ResNet-34 Calcium Scoring experiments..."

# Base command template
BASE_CMD="$PYTHON_PATH run_experiments.py \
    --config_csv experimentation_plan_simplified.csv \
    --biomarker_config config/biomarker_config_calcium.yaml \
    --data_dir /lfs/turing1/0/mahmedc/Comorbidities-Detection/datasets/full_data \
    --model_name \"$MODEL_NAME\" \
    --output_base_dir /lfs/turing1/0/mahmedc/Comorbidities-Detection/models/calcium_only \
    --resume \
    --no_confirm \
    --epochs 100"
...
for i in "${!LEARNING_RATES[@]}"; do
    ...
    SESSION_NAME="resnet_34_calcium_lr_${LR}_gpu_${GPU}"
    ...
    tmux send-keys -t "$SESSION_NAME" "echo \"🚀 Starting ResNet-34 Calcium Scoring - LR: $LR, GPU: $GPU\"" Enter
...
echo "  tmux list-sessions | grep resnet_34_calcium"
echo ""
...
echo ""
echo "📁 Results will be saved in: /lfs/turing1/0/mahmedc/Comorbidities-Detection/models/calcium_only"
