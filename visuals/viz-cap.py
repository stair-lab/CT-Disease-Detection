        ...
        if filename.endswith(".png"):
            try:
                ...
                for i in range(len(classes)):
                    print(classes[i])
                    attributions_occ = occlusion.attribute(input, strides=(3, 8, 8), target=i,
                                                           sliding_window_shapes=(3, 15, 15), baselines=0)
                    fig_tup = viz.visualize_image_attr_multiple(
                        np.transpose(attributions_occ.squeeze().cpu().detach().numpy(), (1, 2, 0)),
                        np.transpose(transformed_img.squeeze().cpu().detach().numpy(), (1, 2, 0)),
                        ["original_image", "heat_map"], ["all", "positive"], show_colorbar=True, outlier_perc=2, )
                    fig1, axis = fig_tup
